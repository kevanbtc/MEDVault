...
curl -s -X POST "http://localhost:3001/api/money/nsa-calculator" \
...
curl -s "http://localhost:3001/api/scripts/provider/site_of_service?serviceCode=99213&facilityType=hospital_outpatient" | \
  jq -r '.script' | head -3
echo ""
